...
# gvcf pon too expensive on time and memory, swith to a bed file with average coverage info (thus, gvcf is recommend but not necessary for a pon)
# adapted for Ming's victorinox, add a sorting step
# v6 only focus on certian panel, use 1000G gvcf sv information for training, all 1000G sv ooverlaping region subset vcf from several gvcf files as a crop for generating positive and negative training data.
# v6.4 expand the reference block, make 3 channel x 5 section ouput csv
# v6.4.2 remove csv after making a jpeg
# v6.4.3 reduce bcftools view step on subbed function
# v6.4.4 set max mem  for bcftools sort
# v6.5 select dbSNP site only to reduce computation burden
...
bcftools annotate -a $gvcf_name.tab.gz -h ann.hdr -c CHROM,POS,relDP,majAF,sGQ $gvcf_file -Oz > $gvcf_name.info.vcf.gz
