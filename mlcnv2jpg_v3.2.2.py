...
for line in Lines: 
    ...
    for i in uniqlist:
        ...
        for j in range(0,img3.shape[1],int(imgsize/2)):
            imloc = int(j)
            ...
            for cnvtype in cnvtypes :
                f1newname = re.sub('.csv','.'+str(i)+'_'+str(j)+'.jpeg',f1name.split('/')[-1].strip())
                im = Image.fromarray(imtemp, mode='RGB')
                im.save(outadd+'/'+outloc+'/'+cnvtype+'/'+f1newname)
            '''
            f1newname = re.sub('.csv','.'+str(i)+'_'+str(imloc)+'.jpeg',f1name.split('/')[-1].strip())
            im = Image.fromarray(imtemp, mode='RGB')
            ...
            if imtempdel + imtempdup > windowthreshold :
                ...
                for mov in [-25,-10,10,25]:
                    if imloc+mov > 0 and imend[-1]+mov < img3.shape[1]:
                        ...
                        '''
                        for cnvtype in cnvtypes :
                            f1newname = re.sub('.csv','.'+str(i)+'_'+str(j+mov)+'.jpeg',f1name.split('/')[-1].strip())
                            im = Image.fromarray(imtemp, mode='RGB')
                            im.save(outadd+'/'+outloc+'/'+cnvtype+'/'+f1newname)
                        '''
